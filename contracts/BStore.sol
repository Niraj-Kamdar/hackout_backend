...
contract BStore {
    ...
    function addBook(uint _id, address _publisher, uint _price, string memory _title, bytes memory _cover) public{
        require(msg.sender == user);
        noOfBooks[msg.sender]++;
        ownedbooks[msg.sender][noOfBooks[msg.sender]] = Book(_id, _publisher, _price, _title, _cover);
    }
}